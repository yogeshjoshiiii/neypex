...
import { getAdminRole, type AdminRole } from "@/lib/admin";
...
// Co-admins can only access these routes
const CO_ALLOWED = new Set(["/admin/payments", "/admin/support"]);
const CO_NAV_KEYS = new Set(["payments", "support"]);

const Gate = ({ children }: { children: (role: AdminRole) => React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    getAdminRole(email).then(setRole);
...
const AdminLayout = () => {
  ...
  return (
    <Gate>
      {(role) => {
        const isCo = role === "co";
        const visibleNav = NAV.filter(n => !isCo || CO_NAV_KEYS.has(n.key));
        const blocked = isCo && !CO_ALLOWED.has(loc.pathname);
        return (
        ...
        <aside className={`fixed md:static z-40 inset-y-0 left-0 w-64 bg-card border-r border-border transform transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
          <div className="h-16 px-5 flex items-center justify-between border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <span className="font-display tracking-wider text-lg">NEYPEX <span className="text-primary">{isCo ? "CO-ADMIN" : "ADMIN"}</span></span>
            </div>
        ...
        <div className="flex-1 min-w-0 md:ml-0">
          <header className="sticky top-0 z-30 h-16 bg-background/80 backdrop-blur border-b border-border flex items-center justify-between px-4 md:px-8">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-display text-xl md:text-2xl tracking-wide">{isCo ? "Co-Admin Dashboard" : "Dashboard"}</h2>
            </div>
            <div className="flex items-center gap-3">
          ...
          >
            {blocked ? <Navigate to="/admin/payments" replace /> : <Outlet context={{ role }} />}
          </motion.main>
        </div>
      </div>
        );
